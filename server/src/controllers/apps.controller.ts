...
const getAppsHandler = async(req: Request<Partial<IGetAppsHandlerRequestParams>, {}, {}, Partial<IAppsFilter>>, res: Response, next: NextFunction) => {
    const {params, query} = req
    const {country, city} = params;
    const filter = query;
    const locationType: LocationType = country ? "country" : "city"
    const location = (country ?? city) as string;
    try {
