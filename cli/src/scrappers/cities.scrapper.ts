...
export class CitiesScrapper {
  ...
  constructor({
    citiesAnswer,
    categoriesAnswer,
  }: ICitiesScrapperConstructorArgs) {
  ...
  public async scrape() {
    ...
    for (const city of this.cities) {
      ...
      for (const [category, subcategories] of entries) {
        ...
        scrapeResults[city]![category]!.apps = await scrapeApps(city, category);
        scrapeResults[city]![category]!.subcategories = {};

        for (const subcategory of subcategories) {
          scrapeResults[city]![category]!.subcategories![subcategory] =
            await scrapeApps(city, subcategory);
        }
      }
    }
    ux.action.stop();
